...
void MainWindow::Init()
{
    ...
    validatorTimes->setRange(1, INT_MAX);
    ui->DelaySendTimes->setValidator(validatorTimes);
    QIntValidator *validatorLen = new QIntValidator(ui->DelaySendTimes);// 随机数据长度编辑限制
    validatorLen->setRange(1, 512);
    ui->DelaySendTimes->setValidator(validatorLen);
...
void MainWindow::ConnectFun()
{
    ...
    connect(ui->ViewLogButton, &QPushButton::clicked, this, [&](){// 查看日志
        // 打开 txt
        QProcess * processLog = new QProcess;
        QString logPath = "notepad.exe log.txt";
        processLog->start(logPath);
    });
    ...
    connect(ui->SendButton, &QPushButton::clicked, this, [&](){// 发送数据
        if (!ui->EditWindow->toPlainText().isEmpty()) {
            ...
            ui->ReceivingWindow->setText(str.append(GetCurTime()).append(" -> ").append(ui->EditWindow->toPlainText()));
...
QByteArray MainWindow::DataAnasis(QString data)
{
    ...
    ans.prepend(QString(length & 255).toLatin1());
    ans.prepend(QString(length >> 8).toLatin1());// 添加长度
    ans.append(XorChecksum(ans));// 添加校验位
...
{
    int result = (int)(data.at(0));
    for (int i = 1; i < data.size(); i++) {
        result ^= (int)(data.at(i));
    }
    qDebug() << result;
    return QString(result).toUtf8();
}
...
void MainWindow::PraseACK()
{
    ...
    int headIndex = dataBuf.indexOf("$ACK");
    if (headIndex != -1)
    dataBuf.remove(0, headIndex);
    //成功的ACK 24 41 43 4B 00 01 01 00
    //失败的ACK 24 41 43 4B 00 01 00 01
    if (hexData.size() >= 15){
    if (hexData.at(11) == '1') {
        if (hexData.at(13) == '1') {
            LogUpdate("接收ACK成功");
...
// 日志更新
void MainWindow::LogUpdate(QByteArray str)
{
    ...
    if (!file.exists()) {// 文件不存在
        if (file.open(QIODevice::WriteOnly)) {
            ...
            out << GetCurTime().append(" ");
            if (str.indexOf('$') != -1)
            {
                buf = str.mid(0, str.indexOf('$'));
                str.remove(0 , str.indexOf('$'));
                file.write(str);
            }
            else
            {
                out << GetCurTime().append(str);
            }
            out << GetCurTime().append(str);
...
void MainWindow::DelaySendData()
{
    ...
    if (this->ui->DelaySend->isChecked()) {// 打开挂机模式
        ...
        RandomDataGenerate(ui->TestDataLength->text().toInt());// 生成随机数据
        mDelayTime = ((ui->TestDataLength->text().toInt()) /986 + 1) * 5000;
        qDebug()<<mDelayTime;
        SendData(DataAnasis(ui->EditWindow->toPlainText()),DataAnasisToLog(ui->EditWindow->toPlainText()));// 发送数据
        mAutoSendCount++;// 发送次数计数
        if (WaitForACK((unsigned int)mDelayTime)) {// 收到 ack 延时
...
void MainWindow::on_flushPort_clicked()
{
    SerialGet();
}
