...
void MainWindow::ConnectFun()
{
    ...
    connect(ui->ViewLogButton, &QPushButton::clicked, this, [&](){// 查看日志
        ...
        if (!file.exists()) {// 文件不存在
            ...
            if (file.open(QIODevice::ReadOnly)) {
                ...
                ui->ReceivingWindow->append(stream.readAll());
                file.close();
            } else {
    ...
    connect(ui->SendButton, &QPushButton::clicked, this, [&](){// 发送数据
        if (!ui->EditWindow->toPlainText().isEmpty()) {
            ...
            SendData(DataAnasis(ui->EditWindow->toPlainText()));
        }
    });
...
QString MainWindow::XorChecksum(const QByteArray &data)
{
    ...
    for (int i = 0; i < data.size(); i++) {
        result ^= static_cast<uchar>(data.at(i));
    }
...
void MainWindow::PraseACK()
{
    ...
    int headIndex = dataBuf.indexOf("2441434b");
    dataBuf.remove(0, headIndex);
    if (hexData.at(13) == '0' && hexData.at(12) == '3') {
        if (hexData.at(11) == '1') {
            LogUpdate("接收ACK成功");
        }
        else if (hexData.at(11) == '0') {
            LogUpdate("接收ACK失败");
        }
        emit ReceivedACK();
    }
...
void MainWindow::DelaySendData()
{
    ...
    if (this->ui->DelaySend->isChecked()) {// 打开挂机模式
        ...
        SendData(DataAnasis(ui->EditWindow->toPlainText()));// 发送数据
        mAutoSendCount++;// 发送次数计数
        if (WaitForACK((unsigned int)10000)) {// 收到 ack 延时
